    ...
    <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom">
        ...
        <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item dropdown">
                @php($adminNote = \App\Models\AdminNote::query()->where('seen',0)->get())
                <a class="nav-link" data-toggle="dropdown" href="#">
                    <i class="fa fa-bell-o"></i>
                    @if($adminNote->count())
                    <span class="badge badge-warning navbar-badge">{{$adminNote->count()}}</span>
                    @endif
                </a>
                <div class="dropdown-menu dropdown-menu-lg dropdown-menu-left">
                    @if($adminNote->count())
                    <span class="dropdown-item dropdown-header">{{$adminNote->count()}} اشعار جديد</span>
                    @endif
                    @forelse($adminNote as $note)
                    <a href="{{url('admin/notes')}}" class="dropdown-item">
                        <i class="fa fa-file ml-2"></i> {{$note->title}}
                    </a>
                    @empty
                    ...
                    <a href="{{url('admin/notes')}}" class="dropdown-item dropdown-footer">مشاهدة كل الاشعارات</a>
                </div>
            </li>
            <li class="nav-item">
